"""Set workshop, saved sets, autoset, and chat routes.

Migrated from Flask routes.py — set workshop operations, saved set CRUD,
autoset narrative builder, and chat conversational AI.
"""
...
@router.post("/api/autoset/build", status_code=202)
...
@router.get("/api/autoset/progress")
...
@router.post("/api/chat/message", status_code=202)
async def chat_message(body: ChatMessageBody, state: AppState = Depends(get_state)):
    ...
    # We need to pass _state-like dict for chat module compatibility
    # Build a dict that the chat module expects
    chat_state = {
        "df": state.df,
